...
cologne.desc = include("ab_src.integrations.eid").cologne
...
cologne:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, function(player, cache_flag)
    if cache_flag == CacheFlag.CACHE_TEARCOLOR then
        player.TearColor = Color(0.867, 0.627, 0.867, 1, 0, 0, 0)
    end
end)

---@param player EntityPlayer
cologne:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, function(player)
    local max_charm_distance = 120 * math.max(player.SpriteScale.X, player.SpriteScale.Y)
    for _, entity in ipairs(Isaac.GetRoomEntities()) do
        if player.Position:Distance(entity.Position) < max_charm_distance and entity:IsVulnerableEnemy() then
