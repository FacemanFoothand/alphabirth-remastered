...
local humility = Item("Humility") ---@type Item
humility.desc = include("ab_src.integrations.eid").humility
humility.Mark = Flag("humility_double_damage") ---@type Flag
humility.Mark.interval = 10
humility.Mark.chance = 10
...
humility:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, function(player)
    ...
    for _, entity in ipairs(Isaac:GetRoomEntities()) do
        if entity:IsActiveEnemy(false) then
            if humility.Mark:EntityHas(entity) then
    ...
    if
        ...
        and valid_entities ~= nil
        and #valid_entities > 0
        and g.game:GetFrameCount() % humility.interval == 0
        and utils.random(1, math.max(1, humility.chance - player.Luck)) <= 1
    then
        ...
        humility.Mark:Apply(target_entity)
    end
end)

---@param player EntityPlayer
humility:AddCallback(ModCallbacks.MC_POST_RENDER, function(player)
    ...
    if save.humility_target then
        ...
        humility.sprite.Offset = save.humility_target:GetSprite().Offset
            - Vector(0, save.humility_target.Size * (save.humility_target.SizeMulti.Y * 3))
...
humility:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, entity, damage_amount)
    if humility.Mark:EntityHas(entity) then
