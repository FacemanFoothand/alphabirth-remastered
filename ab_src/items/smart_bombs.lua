...
smart_bombs.TearFlag = Flag("smart_bomb_flag") ---@type Flag
...
smart_bombs:AddCallback(ModCallbacks.MC_POST_BOMB_INIT, function(_, bomb)
    local room = g.room
    if bomb.SpawnerType == EntityType.ENTITY_PLAYER and not smart_bombs.TearFlag:EntityHas(bomb) and room:IsClear() then
        ...
        if target_entity ~= nil then
            if (not target_entity:ToRock()) and target_entity.State == 2 then
                ...
                smart_bombs.TearFlag:Apply(bomb)
...
smart_bombs:AddCallback(ModCallbacks.MC_POST_BOMB_UPDATE, function(_, bomb)
    if smart_bombs.TearFlag:EntityHas(bomb) then
